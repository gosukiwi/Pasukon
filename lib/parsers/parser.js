...
module.exports = class Parser {
  ...
  buildRules (rules) {
    ...
    rules.forEach((rule) => {
      ...
      this.parsers[rule.name] = this.decorate(new Or(parsers, undefined, rule.name))
      lastRule = rule.name
    })
  ...
  buildRuleCall (rule, definition) {
    ...
    return this.decorate(new Identity(getParser, definition.code))
  }
  ...
  buildBinaryCall (rule, definition) {
    ...
    return this.decorate(new Combinator([lhs, rhs], definition.code))
  }
  ...
  buildUnaryCall (rule, definition) {
    if (definition.parser === 'token') {
      ...
      return this.decorate(new Token(definition.arg.name, definition.code))
    }
    ...
    return this.decorate(new Combinator(this.parserFor(rule, definition.arg), definition.code))
  }
  ...
  decorate (parser) {
    ...
    if (this.options.cache) {
      final = new MemoizableParser(final)
    }
